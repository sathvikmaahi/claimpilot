...
async def main():
    # --- Phase 1: extract (read + LLM, no writes) ---
    result = await extract(
        medicaid_id=MARCUS_MEDICAID_ID,
        narration_activities=_read(NARRATION_ACTIVITIES),
        narration_engagement=None,  # single combined clip; HOW is captured within it
        toggled={field: _read(path) for field, path in TOGGLED_FILES.items()},
    )
    print("=== EXTRACT RESULT ===")
    print(json.dumps(result, indent=2))

    # --- Phase 2: submit (atomic note + MAR write) ---
    # In the app the DSP reviews/edits the note and taps the MAR between these
    # phases. Here we just approve the extraction as-is and confirm the meds.
    ...
    # Self-cleaning so local runs don't litter the DB (MAR cascades on delete).
    delete_care_session(out["care_session_id"])
    print(f"\nCleaned up demo session {out['care_session_id']}")
    return out
