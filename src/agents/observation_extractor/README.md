...
- The target topic is supplied per call via `build_observation_extractor(field)`, which stamps the right instruction and field into a fresh agent. This is what keeps each call disciplined to a single topic, rather than the model grabbing everything it hears.
- Topics the DSP did not toggle are never sent here — they simply stay null. No toggle, no agent call.
- No goal mapping or gap detection happens here; that is the narrative extractor's job.

## Files

| File | Role |
|------|------|
| `agent.py` | The agent factory (`build_observation_extractor`), schema, and instruction. |
